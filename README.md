# Лад — хаб режимов

Входная страница: выбери режим.

- Быстрый: https://alyosha1988.github.io/lad-v1/
- Песня: https://alyosha1988.github.io/lad-v2/
- Рифф: https://alyosha1988.github.io/lad-v3/
- Хаб: https://alyosha1988.github.io/lad/

## Ассеты

- `assets/hero-frets.jpg` (+ `.webp`) — крупный план ладов для hero.
  - **Источник:** сгенерированное фотореалистичное изображение (Cursor GenerateImage). Оригинальная композиция в духе премиального грифа (тёмный гриф / палисандр, полированные лады, студийный свет), без логотипов и без копирования кадров с prsguitars.com.
  - **Лицензия:** создано для проекта «Лад»; не сток Unsplash/Pexels и не маркетинг PRS.
